...
Where the [ICSE-NIER paper](/publications/2026/04/15/icse-nier-model-cards.html) argued the principle, this one runs the experiment at full size. The paper takes the GPT-5 model and system card, pulls out 74 claims, and traces them back to 22 requirements nobody ever wrote down. The requirements that turn out to be missing, and the claims left with no evidence attached, are the actual result: current documentation practice hides both.
{: .text-justify }
